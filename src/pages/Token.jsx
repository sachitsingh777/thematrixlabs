import { Box, Button, Flex, Grid, GridItem, HStack, Input, InputGroup, InputRightElement, SimpleGrid, Text } from '@chakra-ui/react'
import React, { useEffect, useState } from 'react'
import Reactangle9398 from "../assests/Rectangle9398.png"
...
export default function Token() {
    ...
    useEffect(()=>{
        fetch(`https://api.dexscreener.com/latest/dex/tokens/0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2`)
            .then((res) => res.json())
            .then((res) => {
                setData(res.pairs.sort((a, b) => b.priceUsd - a.priceUsd));
            })
    ...
    return (
        <Box w="1512px" display={'flex'} gap={5} flexDir={{ base: 'column', sm: 'column', md: 'column', lg: 'row' }} backgroundImage={Reactangle9398}>
            ...
            <Box w={['95%', '95%', '95%', '80%']} m={'auto'}>
                <Flex mt={'27px'} mb={'15px'} mx={3} alignItems={'center'} justifyContent={'space-between'}>
                    <InputGroup className='poppins18' w={['90%', '90%', '90%', '435px']} h={'50px'} flexShrink={0}>
                        <Input borderRadius={'20px'} color={'white'} border={'1px solid #FFF'} background={' linear-gradient(95deg, #7C0F35 7.59%, #581266 104.01%)'}
                    ...
                    <Button
                        ...
                        h={'52px'}
                        borderRadius={'20px'}
                        background={' linear-gradient(95deg, #7C0F35 7.59%, #581266 104.01%)'}
                        color={'white'}
                        _hover={{
                            background: ' linear-gradient(95deg, #7C0F35 7.59%, #581266 104.01%)'
                        }}
                ...
                <Box height={'850px'}
                    ...
                    <Text className='worksans' fontWeight={600} fontSize="3xl" color="whiteAlpha.800">Token Search Results</Text>
                    <Grid templateColumns={{ base: "1fr", sm: "1fr", md: "1fr 1fr", lg: "1fr 1fr 1fr 1fr" }} gap={3}>
                        {data?.map((el, i) => (

                            <>
                                <Box key={i} m={'auto'} w={'279px'} h={'188px'} mt={'10px'} borderRadius={'10px'} background={'#390554'}>
